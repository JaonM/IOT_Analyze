    ...
    for s in data['eui'].unique():
        ...
        for date in date_range:
            ...
            if len(sub_sensor['传感器名称']) > 0:
                try:
                    sub_sensor.plot(y=['temperature', 'humidity', 'power', 'voltage', 'current', 'battery'], x='times',
                                    subplots=False, figsize=(12, 8), title=str(s))
                    # fig = ax[0].get_figure()
                    # fig.savefig(date + ' ' + str(eui) + ' ' +
                    #             sub_sensor['传感器名称'][0] + '.jpg')
                    plt.savefig('../' + save_folder + '/' + date + ' ' + s + '.png')
                except TypeError:
                ...
                try:
                    ...
                    plt.savefig('../' + save_folder + '/' + date + ' ' + str(s) + '.png')
                except TypeError:
                    print('empty dataframe')
...
draw('2018-01-29.xlsx', ['2018-01-29'])
